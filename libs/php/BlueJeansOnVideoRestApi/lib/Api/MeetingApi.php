...
class MeetingApi
{
     ...
     * @param \BlueJeansOnVideoRestApi\Model\PayloadDialout $payload_dialout  (required)
     * @throws \BlueJeansOnVideoRestApi\ApiException on non-2xx response
     * @return \BlueJeansOnVideoRestApi\Model\DialoutPstn[]
     */
     ...
     * @param \BlueJeansOnVideoRestApi\Model\PayloadDialout $payload_dialout  (required)
     * @throws \BlueJeansOnVideoRestApi\ApiException on non-2xx response
     * @return array of \BlueJeansOnVideoRestApi\Model\DialoutPstn[], HTTP status code, HTTP response headers (array of strings)
     */
    public function dialoutPstnWithHttpInfo($user_id, $numeric_meeting_id, $payload_dialout)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\BlueJeansOnVideoRestApi\Model\DialoutPstn[]',
                '/v1/user/{user_id}/live_meetings/{numeric_meeting_id}/dialout/pstn'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\BlueJeansOnVideoRestApi\Model\DialoutPstn[]', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\BlueJeansOnVideoRestApi\Model\DialoutPstn[]', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                default:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\BlueJeansOnVideoRestApi\Model\Error', $e->getResponseHeaders());
     ...
     * @param int $user_id The ID of the user of interest. This value is an integer which can be retrieved for the current user via the Get User Account Details endpoint. (required)
     * @param int $numeric_meeting_id The meeting ID that participants will see and use to join the conference. When joining via phone, this is the code they enter via DTMF to join. (required)
     * @param \BlueJeansOnVideoRestApi\Model\PayloadPairingCodePstn $payload_pairing_code_pstn  (required)
