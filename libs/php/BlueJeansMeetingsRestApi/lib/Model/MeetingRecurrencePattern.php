 ...
 * @link        https://github.com/swagger-api/swagger-codegen
 */
class MeetingRecurrencePattern implements ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        $allowed_values = $this->getRecurrenceTypeAllowableValues();
        if (!in_array($this->container['recurrence_type'], $allowed_values)) {
        ...
        $allowed_values = $this->getWeekOfMonthAllowableValues();
        if (!in_array($this->container['week_of_month'], $allowed_values)) {
            $invalid_properties[] = sprintf(
        ...
        $allowed_values = $this->getMonthOfYearAllowableValues();
        if (!in_array($this->container['month_of_year'], $allowed_values)) {
            $invalid_properties[] = sprintf(
    ...
    {
        $allowed_values = $this->getRecurrenceTypeAllowableValues();
        if (!is_null($recurrence_type) && !in_array($recurrence_type, $allowed_values)) {
    ...
    {
        $allowed_values = $this->getWeekOfMonthAllowableValues();
        if (!is_null($week_of_month) && !in_array($week_of_month, $allowed_values)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'week_of_month', must be one of '%s'",
                    implode("', '", $allowed_values)
    ...
    {
        $allowed_values = $this->getMonthOfYearAllowableValues();
        if (!is_null($month_of_year) && !in_array($month_of_year, $allowed_values)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'month_of_year', must be one of '%s'",
                    implode("', '", $allowed_values)
