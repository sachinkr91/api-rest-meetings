...
authSvc.getTokenByPassword(
	...
	function (err, grant)
	{
		...
		userSvc.updateUser(grant.scope.user, {"title": "Rule of Everything"}, function (err, user)
		{
			if (err) console.log(err);
			if (err) return;

			// Print some arbitrary information
			console.log('Hello ' + user.firstName  + ' ' + user.lastName + ', you are now known as ' + user.title + '!');
		});
	}
);